...
- [ ] §9 Conversation recap과 자가평가를 답변 **맨 끝**에 넣었는가

---

## §9 Conversation recap + Response footer (MUST)

**모든 assistant 응답** 맨 끝에 사용자 관점의 `Conversation recap`을 1~3문장으로 먼저 남기고, 그 아래에 에이전트 자가평가를 한 줄로 남긴다. recap에는 이번 턴에서 끝난 일·남은 일·차단 사항만 쓴다. 작업 중 사용한 내부 절차·도구·추론은 반복하지 않는다.

```text
─ Conversation recap ──────────────────────────────────────
{이번 턴 완료·남은 일·차단 사항 1~3문장}

inquiry-first (자가평가): {등급} (확신도: {점수}/10) — {상태 한 줄} [응답 ID: {응답_ID}]
```

recap과 자가평가 모두 생략하지 않는다. 사용자에게 이미 답변 본문으로 충분히 전달한 내용을 recap에서 장황하게 반복하지 않는다.
...
```text
─ Conversation recap ──────────────────────────────────────
설정 테스트의 실제 API 호출을 mock으로 격리했고, 전체 테스트가 통과했다. 커밋·푸시는 아직 하지 않았다.

inquiry-first (자가평가): Low (확신도: 10/10) — 테스트 수정·검증 완료, 커밋·푸시는 미수행 [응답 ID: example-01]
```

**예외 (recap + footer 생략 가능)**

- 사용자가 "footer/recap 생략" 등 명시
